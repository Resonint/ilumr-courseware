...
PARDEF = [
    ...
    ParDef('n_ETL', int, 0),
    ParDef('t_rep', float, 1),
    ParDef('t_echo', float, 2e-3),
...
def main(p: ParameterSet):
    ...
    t_echo_min = p.t_180 + 4*p.t_grad_ramp + 2*p.t_phase + p.t_read + 2*t_echo_pad

    t_echo_pad += (p.t_echo-t_echo_min)/2

    if t_echo_pad < t_MIN:
        raise Exception('t_echo too short!')
    
    log.debug(f"echo padding time: {t_echo_pad}")
    
    t_phase_read = p.t_read/2 - p.t_grad_ramp/2
    log.debug(f"t_phase_read: {t_phase_read*1e6} us")
    t_phase_read_180 = (p.t_echo - p.t_90 - p.t_180)/2 - t_phase_read - 3*p.t_grad_ramp
    ...
    t_cpmg = p.t_90/2 + p.t_echo/2 + n_echos*p.t_echo - p.t_180/2 + p.t_spoil
    if p.enable_IR:
        t_cpmg += p.t_180/2 + p.t_inv - p.t_90/2
    #t_rep = t_cpmg + t_end
    t_end = p.t_rep-t_cpmg
    if t_end<0:
        raise Exception('t_rep too short')
    ...
    grad_duty_cycle = grad_total_area/(n_runs*p.t_rep)
    ...
    for i_scan in range(-1 if p.enable_dummy_run else 0, p.n_scans):
