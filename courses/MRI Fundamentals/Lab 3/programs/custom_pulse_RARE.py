...
def main(p: ParameterSet):
    ...
    p_rf_acc = 0 # total accumation of RF phase by running at a different frequency
    ...
    for i_scan in range(p.n_scans):
        ...
        for i_run in range(n_runs):
            ...
            # undo RF DDS phase evolution for this run by running at negative offset for total pulse duration
            yield seq.pulse_update(p.f-p.f_slice_offset, 0, 0)
            yield seq.wait(p.t_90 + 20e-9 + n_echos*p.t_180 + 1e-6) # TODO: work out why extra 1us is needed
            yield seq.pulse_update(p.f, 0, 0)
            p_rf_acc = 0
