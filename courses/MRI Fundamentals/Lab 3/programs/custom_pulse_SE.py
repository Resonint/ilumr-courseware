...
def main(par: ParameterSet):
    ...
    for i_scan in range(par.n_scans):
        ...
        yield seq.pulse_start(par.f, 0, 0) # enable RF switch early to avoid minor switch delay effect
        yield gen_grad_ramp(g_ZERO, par.g_slice, par.t_grad_ramp, par.n_grad_ramp)
        
        yield seq.pulse_update(par.f, p_90, shape_90_scaled[0])
        ...
        yield gen_grad_ramp(g_ZERO, -par.g_slice, par.t_grad_ramp, par.n_grad_ramp)
        yield seq.wait(par.t_90/2-par.t_grad_ramp/2+15e-6) # TODO: fix timing error when t_90 < t_grad_ramp
        yield gen_grad_ramp(-par.g_slice, g_ZERO, par.t_grad_ramp, par.n_grad_ramp)
        ...
        yield seq.wait(t_read/2-par.t_grad_ramp/2-5e-6) # TODO: work out how to tweak this reliably
